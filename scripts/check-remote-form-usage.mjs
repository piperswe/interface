...
for (const file of svelteFiles) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    // Track loop depth
    if (LOOP_RE.test(line)) { inLoop = true; loopDepth++; continue; }
    ...
    if (forMatch) {
      ...
      if (forPart) {
        ...
        if (/^'[^']*'$/.test(keyArg) || /^"[^"]*"$/.test(keyArg)) {
          if (!keyArg.includes('${')) {
            console.error(`\x1b[33mWARNING\x1b[0m: ${file}:${i + 1} — \`.for("${keyArg}")\` inside {#each} uses a static key.\n  Each iteration needs a different key; consider using a dynamic value like \`{p.id}\`.`);
          }
        }
      }
    }
