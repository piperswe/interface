...
describe('buildShellCommand', () => {
	it('exports valid POSIX env var names with safely-quoted values', () => {
		const script = _buildShellCommand('whoami', {
			env: { _UNDERSCORE: 'a "b" c', FOO_BAR: 'baz' },
		});
	...
	it('rejects env keys that contain shell metacharacters', () => {
		...
		expect(() => _buildShellCommand('echo', { env: { 'FOO;rm -rf /': 'bar' } })).toThrow(/Invalid env var name/);
	});

	it('rejects env keys starting with a digit', () => {
		expect(() => _buildShellCommand('echo', { env: { '1FOO': 'bar' } })).toThrow(/Invalid env var name/);
	});

	it('rejects env keys with a dollar sign', () => {
		expect(() => _buildShellCommand('echo', { env: { FOO$BAR: 'baz' } })).toThrow(/Invalid env var name/);
	});
});
