...
export async function getFlyMachineId(env: Env, conversationId: string): Promise<string | null> {
	const row = await env.DB.prepare(`SELECT external_id FROM conversation_sandbox WHERE conversation_id = ? AND backend = 'fly'`)
		.bind(conversationId)
		.first<{ external_id: string | null }>();
	return row?.external_id ?? null;
}

export async function setFlyMachineId(env: Env, conversationId: string, machineId: string): Promise<void> {
	const now = nowMs();
...
export async function clearFlyMachineId(env: Env, conversationId: string): Promise<void> {
	await env.DB.prepare(`DELETE FROM conversation_sandbox WHERE conversation_id = ? AND backend = 'fly'`).bind(conversationId).run();
}

export async function recordExposedPort(env: Env, conversationId: string, port: number, token: string): Promise<void> {
	const now = nowMs();
	await env.DB.prepare(
...
export async function listExposedPorts(env: Env, conversationId: string, hostname: string): Promise<ExposedPort[]> {
	const res = await env.DB.prepare(`SELECT port, token FROM conversation_exposed_ports WHERE conversation_id = ?`)
		.bind(conversationId)
...
export async function clearExposedPorts(env: Env, conversationId: string): Promise<void> {
	await env.DB.prepare(`DELETE FROM conversation_exposed_ports WHERE conversation_id = ?`).bind(conversationId).run();
}
