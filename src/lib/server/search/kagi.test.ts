...
describe('KagiSearchBackend', () => {
	...
	it('parses result items, dropping related-search rows', async () => {
		vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
			new Response(
				JSON.stringify({
					data: [
						{ t: 0, url: 'https://a', title: 'A', snippet: 'sa', published: '2099-01-01' },
						{ t: 1, url: 'related-search-url' },
						{ t: 0, url: 'https://b', title: 'B', snippet: 'sb' },
					],
				}),
				{ status: 200 },
			),
			...
			new Response(
				JSON.stringify({
					data: [
						...
						{ t: 0, title: 'B' /* no url */ },
						{ t: 0, url: 'https://c', title: 'C', snippet: 'sc' },
					],
				}),
				{ status: 200 },
			),
	...
	it('reports api errors when the body has an error array', async () => {
		vi.spyOn(globalThis, 'fetch').mockResolvedValueOnce(
			new Response(JSON.stringify({ error: [{ code: 1, msg: 'rate limited' }] }), { status: 200 }),
		);
		const out = await new KagiSearchBackend('s').search('q');
		expect(out.ok).toBe(false);
