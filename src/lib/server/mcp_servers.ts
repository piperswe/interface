...
export async function listMcpServers(env: Env, userId: number = SINGLE_USER_ID): Promise<McpServerRow[]> {
	const result = await env.DB.prepare(`SELECT ${SELECT_COLS} FROM mcp_servers WHERE user_id = ? ORDER BY name`).bind(userId).all<Row>();
	return (result.results ?? []).map(rowToServer);
}

export async function getMcpServer(env: Env, id: number): Promise<McpServerRow | null> {
	const row = await env.DB.prepare(`SELECT ${SELECT_COLS} FROM mcp_servers WHERE id = ?`).bind(id).first<Row>();
...
export async function setMcpServerOauthTokens(
	...
	const sql = reEnable
		...
		: `UPDATE mcp_servers
		 ...
		 WHERE id = ?`;
	await env.DB.prepare(sql).bind(tokens.accessToken, tokens.refreshToken, tokens.expiresAt, id).run();
}
...
export async function setMcpServerOauthClient(env: Env, id: number, reg: OauthClientRegistration): Promise<void> {
...
export async function createMcpServer(env: Env, input: CreateMcpServerInput, userId: number = SINGLE_USER_ID): Promise<number> {
	const now = nowMs();
	const result = await env.DB.prepare(
		...
		.bind(userId, input.name, input.transport, input.url ?? null, input.command ?? null, input.envJson ?? null, input.authJson ?? null, now)
		.first<{ id: number }>();
	return result?.id ?? 0;
}
...
export async function setMcpServerEnabled(env: Env, id: number, enabled: boolean, userId: number = SINGLE_USER_ID): Promise<void> {
